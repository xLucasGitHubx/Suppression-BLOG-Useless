#!/usr/bin/env node
/**
 * Lance le flow OAuth2 Google une seule fois.
 * Ouvre le navigateur → tu autorises → le token est sauvegardé dans data/google-token.json.
 * Les runs suivants utilisent ce token (auto-refresh).
 *
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    res.end(`
      ...
      <p>Token sauvegardé. Tu peux fermer cette fenêtre et relancer <code>npm run audit</code>.</p>
    `);
    server.close();

    console.log(`✅ Token sauvegardé dans ${TOKEN_PATH}`);
    console.log("   Lance maintenant : npm run audit");
    process.exit(0);
  } catch (err) {
